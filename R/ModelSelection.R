...
#' @export
#'
RunCellMatch = function(query, reference, results_path, K = 2000, num_init = 5, verbose = T){
  query %<>% as.matrix
  reference %<>% as.matrix
  ...
  matchmodel = cellmatch::SelectModelThoroughly(query     = query[variable_genes,],
...
#' @export
#'
SelectModelThoroughly = function(  query,
                                   reference,
                                   compute_penalty = "correlation_distance",
                                   penalty_is_genewise = T,
                                   values = colnames(reference),
  ...
  for(ii in 1:num_init){
    current = SelectModelFast(query = query,
                              reference = reference,
                              compute_penalty = compute_penalty,
...
#' @export
#'
SelectModelFast = function(  query,
                             reference,
                             compute_penalty = "correlation_distance",
                             penalty_is_genewise = T,
                             values = colnames(reference),
